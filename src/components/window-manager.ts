namespace cclib {
    ...
    export class WindowManager {
         ...
         * @returns
         */
        async add(path: string, groupName: string, data: any = null) {
            try {
                let prefab = await resMgr.loadAsset<cc.Prefab>(path, cc.Prefab, groupName);
                let node = cc.instantiate(prefab);
                let win = node.getComponent(Window);
                if (!win) {
                    resMgr.releaseAsset(path);
        ...
        remove(path: string, releasePrefab: boolean = true) {
            try {
                ...
                if (releasePrefab) resMgr.releaseAsset(path);
        ...
        async pushPanel(path: string, groupName: string, data: any = null) {
            try {
                let prevNode = this._panelNode.children[0];
                let prevPanel = prevNode?.getComponent(Panel);
                let node = await this.add(path, groupName, data);
